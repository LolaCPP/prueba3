...
def extract_config_objects(args: argparse.Namespace, groups: dict[str, argparse._ArgumentGroup]) -> dict[str, Any]:
    ...
    if "training" in groups:
        training_args = get_group_args(args, groups["training"])
        training_args["model_id"] = args.model_id
        configs["training"] = TrainingConfig(**training_args)
    ...
    if "dataset" in groups:
        dataset_args = get_group_args(args, groups["dataset"])
        dataset_args["dataset"] = args.dataset
        dataset_args["output_dir"] = args.output_dir

        # Si usas --dataset pero el código espera dataset_dir, lo copiamos también.
        if dataset_args.get("dataset_dir") is None:
            dataset_args["dataset_dir"] = args.dataset

        configs["dataset"] = FileConfig(**dataset_args)
